...
import { getHsnwiseReport } from '../../db/queries';
import type { HsnwiseRow } from '../../types';

export default function HsnCodeReport() {
  const db = useSQLiteContext();
  const [rows, setRows] = useState<HsnwiseRow[]>([]);
  const [from, setFrom] = useState('');
  const [to, setTo] = useState('');

  const load = useCallback(() => {
    getHsnwiseReport(db, { from: from || undefined, to: to || undefined }).then(setRows);
  }, [db, from, to]);
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        data={rows}
        keyExtractor={(r) => r.hsn_code}
        contentContainerStyle={{ padding: 16, paddingTop: 4 }}
        ListEmptyComponent={<Text style={styles.empty}>No items yet.</Text>}
        renderItem={({ item }) => (
          <View style={styles.card}>
            <Text style={styles.name}>{item.hsn_code}</Text>
            <Text style={styles.meta}>{item.item_count} item(s)</Text>
